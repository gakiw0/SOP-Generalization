...
class BaseballPlugin(BaseRulePlugin):
    ...
    def compute_metrics(self, phase_id: str, student_data: np.ndarray, coach_data: np.ndarray) -> Dict[str, float]:
        ...
        if phase_id == "step1":
            metrics["stance_angle_diff_ratio"] = stance_angle_diff_ratio(student_data, coach_data)
            metrics["cg_z_avg_ratio_or_flag"] = cg_z_avg_ratio_or_flag(student_data, coach_data)
        elif phase_id == "step2":
            metrics["head_move_diff_ratio"] = head_move_diff_ratio(student_data, coach_data)
            metrics["stride_z_class"] = stride_z_class(student_data, coach_data)
            metrics["cg_z_end_ratio_or_flag"] = cg_z_end_ratio_or_flag(student_data, coach_data)
            metrics["shoulder_xz_angle_diff_ratio"] = shoulder_xz_angle_diff_ratio(student_data, coach_data)
        elif phase_id == "step3":
            metrics["cg_z_end_diff_class"] = cg_z_end_diff_class(student_data, coach_data)
            metrics["shoulder_height_diff_class"] = shoulder_height_diff_class(student_data, coach_data)
        elif phase_id == "step4":
            metrics["cg_z_std_diff_ratio"] = cg_z_std_diff_ratio(student_data, coach_data)
            metrics["hip_yaw_angle_diff_ratio_or_clamp"] = hip_yaw_angle_diff_ratio_or_clamp(student_data, coach_data)
        else:
