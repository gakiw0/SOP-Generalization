import { useTranslation } from 'react-i18next'
import type { StepDraft } from '../draftTypes'

type StepListProps = {
  steps: StepDraft[]
  selectedStepId: string | null
  onSelect: (stepId: string) => void
  onAdd: () => void
  onRemove: (stepId: string) => void
}

export function StepList({ steps, selectedStepId, onSelect, onAdd, onRemove }: StepListProps) {
  const { t } = useTranslation()

  return (
    <section className="cb-panel cb-step-sidebar">
      <div className="cb-panel-header">
        <h2>
          {t('step.listTitle')}
          <span className="cb-title-badge">{steps.length}</span>
        </h2>
        <button type="button" onClick={onAdd} data-testid="cb-steps-add">
          {t('step.addButton')}
        </button>
      </div>
      <p>{t('step.listHelp')}</p>

      <ul className="cb-step-list">
        {steps.map((step, index) => {
          const selected = step.id === selectedStepId
          const checkpointCount = step.checkpoints.length
          return (
            <li key={step.id} className={selected ? 'is-selected cb-step-row' : 'cb-step-row'}>
              <button
                type="button"
                onClick={() => onSelect(step.id)}
                className="cb-step-card"
                data-testid={`cb-steps-select-${index}`}
              >
                <span className="cb-step-title-row">
                  <span className="cb-step-index">#{index + 1}</span>
                  <span className="cb-step-title">{step.label || step.id}</span>
                </span>
                <span className="cb-step-meta">{step.category || t('step.uncategorized')}</span>
                <span className="cb-step-submeta">
                  {t('step.checkpointCount', { count: checkpointCount })}
                </span>
              </button>
              <button
                type="button"
                className="cb-danger"
                onClick={() => onRemove(step.id)}
                disabled={steps.length <= 1}
                data-testid={`cb-steps-remove-${index}`}
              >
                {t('common.remove')}
              </button>
            </li>
          )
        })}
      </ul>
    </section>
  )
}
